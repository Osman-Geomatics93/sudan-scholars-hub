              ...
              materials.map((mat) => (
                <div
                  ...
                  className={`cursor-pointer p-4 transition-colors hover:bg-gray-50 ${
                    selectedMaterial?.id === mat.id ? 'bg-primary-50' : ''
                  }`}
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        <h3 className="font-semibold text-gray-900 truncate">{mat.title}</h3>
                      ...
                      <p className="text-sm text-gray-600 mb-1">{mat.subject}</p>
          ...
          {selectedMaterial ? (
            <div>
              <div className="mb-6 border-b pb-4">
                ...
                <p className="text-sm text-gray-600">{selectedMaterial.subject}</p>
              </div>

              <dl className="space-y-3 text-sm">
                <div className="flex justify-between">
                  <dt className="text-gray-500">{isRTL ? 'النوع' : 'Type'}</dt>
                ...
                <div className="flex justify-between">
                  ...
                  <dd className="text-gray-900">{isRTL ? roleLabels[selectedMaterial.uploaderRole]?.ar : roleLabels[selectedMaterial.uploaderRole]?.en}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-gray-500">{isRTL ? 'الدولة' : 'Country'}</dt>
                  <dd className="text-gray-900">{selectedMaterial.countryName}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-gray-500">{isRTL ? 'الجامعة' : 'University'}</dt>
                  <dd className="text-gray-900">{selectedMaterial.universityName}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-gray-500">{isRTL ? 'المرحلة' : 'Degree'}</dt>
                  <dd className="text-gray-900">{selectedMaterial.degreeName}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-gray-500">{isRTL ? 'الفصل' : 'Semester'}</dt>
                  <dd className="text-gray-900">{selectedMaterial.semester}</dd>
                </div>
                {selectedMaterial.facultyId && (
                  <div className="flex justify-between">
                    <dt className="text-gray-500">{isRTL ? 'الكلية' : 'Faculty'}</dt>
                    <dd className="text-gray-900">{selectedMaterial.facultyId}</dd>
                  </div>
                )}
                {selectedMaterial.specialtyId && (
                  <div className="flex justify-between">
                    <dt className="text-gray-500">{isRTL ? 'التخصص' : 'Specialty'}</dt>
                    <dd className="text-gray-900">{selectedMaterial.specialtyId}</dd>
                  </div>
                )}
                <div className="flex justify-between">
                  <dt className="text-gray-500">{isRTL ? 'تاريخ الرفع' : 'Uploaded'}</dt>
                  <dd className="text-gray-900">{formatDate(selectedMaterial.uploadedAt)}</dd>
                </div>
                {selectedMaterial.reviewedAt && (
                  <div className="flex justify-between">
                    <dt className="text-gray-500">{isRTL ? 'تاريخ المراجعة' : 'Reviewed'}</dt>
                    <dd className="text-gray-900">{formatDate(selectedMaterial.reviewedAt)}</dd>
                  </div>
                )}
                {selectedMaterial.description && (
                  <div>
                    <dt className="text-gray-500 mb-1">{isRTL ? 'الوصف' : 'Description'}</dt>
                    <dd className="text-gray-900 whitespace-pre-wrap bg-gray-50 rounded-lg p-3 text-sm">{selectedMaterial.description}</dd>
                  </div>
                )}
                {selectedMaterial.rejectionNote && (
                  <div>
                    <dt className="text-red-500 mb-1">{isRTL ? 'سبب الرفض' : 'Rejection Note'}</dt>
                    <dd className="text-red-700 bg-red-50 rounded-lg p-3 text-sm">{selectedMaterial.rejectionNote}</dd>
                  </div>
                )}
              </dl>
